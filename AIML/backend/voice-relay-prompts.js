...
export const SPOKEN = {
  ...
  codingWbIntro(type, variant = "start") {
    return {
  ...
  greeting(aiName, title, count, spokenQuestion) {
    return {
  ...
  transition: {
    ...
    normal(qNum, qText, optionsSuffix) {
      return {
...
export const PROMPTS = {
  ...
  summarize(questionText, transcript) {
    return {
...
Note: ONLY categories 4 and 5 may include ${nextToken}. ONLY category 6 may include ${prevToken}. All others must NEVER include either token.`,
    ...
    whiteboard(nextToken, prevToken) {
      return {
