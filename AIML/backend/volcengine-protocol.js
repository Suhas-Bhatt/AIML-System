...
export function buildStartConnection() {
...
export function buildFinishConnection() {
  const header = generateHeader();
  const eventId = Buffer.alloc(4);
...
export function buildStartSession(
  ...
  const dialog = { bot_name: botName.slice(0, 20) };
  if (systemText) {
...
export function buildFinishSession(sessionId) {
  const header = generateHeader();
  const eventId = Buffer.alloc(4);
...
export function parseResponse(data) {
  ...
  const result = { messageType, payload: null };
  let offset = 0;
