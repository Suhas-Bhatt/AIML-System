...
import { getProvider, GENERATOR_MODEL } from '../../../../lib/ai/registry.js';
import { createLogger } from '../../../../lib/logger.js';
import { buildGeneratorPrompt } from '../../../../lib/ai/prompts/generator.js';
...
export async function POST(req) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const collectLlmContent = async (streamToClient) => {
        let inThink = false;
        let fullContent = "";
      ...
      try {
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            generated = parseJsonSafe(fullContent);
            break;
          } catch (error) {
            lastError = error;
