import { getLanguageKey, LANGUAGE_DISPLAY_NAME } from '../../../lib/i18n.js';
...
export function buildSummaryPrompt(
  ...
  const candidateSection = candidate
    ? `\nCandidate Background:\n- Name: ${candidate.name || "Unknown"}\n- Experience: ${candidate.workExperience || "Not provided"}\n- Education: ${candidate.education || "Not provided"}\n- Resume Notes: ${candidate.notes || "Not provided"}`
    : "";

  const systemPrompt = `You are an expert interview analyst. Evaluate and summarize the following interview transcript. Focus EXCLUSIVELY on the participant's responses — their depth, relevance, and quality.

### CRITICAL INSTRUCTIONS FOR SCORING:
- If the participant provided NO answers (empty transcript for user) or only minimal filler (e.g., "hello", "I don't know", "um"), you MUST assign a score of 0 or 1 for that question/criterion.
- DO NOT hallucinate answers or give credit for the context provided by the interviewer's questions. 
- If the overall engagement is extremely low, the summary MUST explicitly state that the participant failed to provide substantial information.
- High scores (8-10) should be reserved for deep, insightful, and comprehensive answers.
- CROSS-VALIDATION: Where applicable, validate the participant's answers against their "Candidate Background" (resume information). Note any discrepancies or alignment.

Interview: "${interviewTitle}"${objectiveSection}${criteriaSection}${questionsSection}${candidateSection}${whiteboardSection}${codeSection}
