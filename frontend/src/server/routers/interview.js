...
import { INTERVIEW_TEMPLATES } from '../../lib/interview-templates.js';
import { getSessionOverallScore } from '../../lib/session-score.js';
...
export const interviewRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input?.projectId) {
        ...
        const allProjIds = (orgProjects ?? []).map((p) => p.id);
        ...
        const allProjIds = (allProjects ?? []).map((p) => p.id);
        const accessibleProjIds = await filterAccessibleProjectIds(
          ctx.supabase,
          allProjIds,
          ctx.user.id,
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        ...interview,
        _count: {
          sessions: interview.sessions?.length ?? 0,
        },
      };
    }),
  ...
  create: protectedProcedure
    .input(
      z.object({
        ...
        aiName: z.string().default("AIML System"),
        aiTone: z.enum(["CASUAL", "PROFESSIONAL", "FORMAL", "FRIENDLY"]).default("PROFESSIONAL"),
        followUpDepth: z.enum(["LIGHT", "MODERATE", "DEEP"]).default("MODERATE"),
    ...
    .mutation(async ({ ctx, input }) => {
      // Resolve project: use provided or fall back to user's default project
      const projectId = input.projectId ?? (await resolveDefaultProject(ctx.supabase, ctx.user.id));
      if (!projectId) {
        throw new TRPCError({
          code: "BAD_REQUEST",
      ...
      assertMinRole(membership.role, "MEMBER");
      ...
      const { data: interview, error } = await ctx.supabase
        ...
        .insert({
          ...
          antiCheatingEnabled: input.antiCheatingEnabled,
          projectId,
          userId:              ctx.user.id,
          requireInvite:       false,
          isActive:            false,
  ...
  deleteMany: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Fetch all interviews with their org info in ONE query
      const { data: interviews } = await ctx.supabase
        .from("interviews")
        .select("id, project:projects!inner(organizationId)")
        .in("id", input.ids);
      ...
      // Collect unique org IDs and verify membership ONCE per org
      const orgIds = [...new Set(
        interviews.map((i) => i.project?.organizationId).filter(Boolean)
      )];

      for (const orgId of orgIds) {
        const membership = await getOrgMembership(ctx.supabase, orgId, ctx.user.id);
        if (!membership) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "You do not have permission to delete these interviews",
          });
        }
      }
  ...
  dashboardStats: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const { data: sessions } = await ctx.supabase
        .from("sessions")
        .select(
          "id, interviewId, status, modeUsed, totalDurationSeconds, themes, createdAt, participantName, participantEmail",
        )
        .in("interviewId", interviewIds);
      ...
      if (allSessions.length > 0) {
        ...
        // Use DB-side count — avoids fetching all rows just to count them
        const { data: msgCounts } = await ctx.supabase
          .from("messages")
          .select("sessionId")
          .in("sessionId", sIds);
        for (const row of msgCounts ?? []) {
          const sid = row.sessionId;
          sessionMsgMap[sid] = (sessionMsgMap[sid] ?? 0) + 1;
          totalMessages++;
        }
      }
      ...
      for (const q of questionRows ?? []) {
        const t = q.type ?? "OPEN_ENDED";
        if (t in questionTypeBreakdown)
          questionTypeBreakdown[t]++;
      }
      ...
      // Recent completed sessions - Fetch insights ONLY for the 5 most recent completed sessions
      let recentSessions = [];
      if (allSessions.length > 0) {
        const { data: recentCompletedRows } = await ctx.supabase
          .from("sessions")
          .select(
            "id, interviewId, status, totalDurationSeconds, createdAt, participantName, participantEmail, insights",
          )
          .in("interviewId", interviewIds)
          .eq("status", "COMPLETED")
        ...
        recentSessions = (recentCompletedRows ?? []).map((s) => {
          const score = getSessionOverallScore(s.insights);
          return {
            id: s.id,
