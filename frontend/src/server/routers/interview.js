...
export const interviewRouter = router({
  ...
  deleteMany: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Fetch all sessions with their org info in ONE query
      ...
      if (!sessions || sessions.length === 0) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Sessions not found" });
      }

      // Collect unique org IDs and verify membership ONCE per org (not per session)
      const orgIds = [...new Set(
        sessions.map((s) => s.interview?.project?.organizationId).filter(Boolean)
      )];
      ...
      for (const orgId of orgIds) {
        ...
        if (!membership) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "You do not have permission to delete these sessions",
          });
        }
      }
