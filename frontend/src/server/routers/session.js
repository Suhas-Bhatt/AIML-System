...
import { z } from "zod";
import { filterAccessibleProjectIds, hasProjectAccess, protectedProcedure, publicProcedure, router } from '../trpc.js';
...
export const sessionRouter = router({
  ...
  createPreview: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const project = interviewAccess.project;
  ...
  getById: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (session.interview?.questions) {
        session.interview.questions.sort((a, b) => a.order - b.order);
      }
  ...
  sendMessage: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Rate limit: max 1 message per 500ms per session to prevent flooding
  ...
  saveWhiteboard: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      let parsed;
      try {
        parsed = JSON.parse(input.snapshotData);
      } catch {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Invalid snapshot data — could not parse JSON",
        });
      }
  ...
  complete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: sessionData } = await ctx.supabase
        .from("sessions")
        .select("*, interview:interviews(*), messages(*)")
        .eq("id", input.id)
        .single();

      if (sessionData) {
        // Fire AI evaluation in the background — do NOT await it
        // This returns immediately to the candidate instead of waiting 5-30s
        aiClient.evaluateSession({
          sessionId: input.id,
          role: sessionData.interview?.target_role || sessionData.interview?.title || "Candidate",
          topic: sessionData.interview?.objective || "Technical Interview",
          messages: sessionData.messages || [],
          proctor_logs: sessionData.antiCheatingLog || [],
        })
          ...
          .catch((e) => {
            log.error("Background AI evaluation failed:", e);
  ...
  listByInterview: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const [membershipRes, projAccess] = await Promise.all([
        ctx.supabase
          .from("organization_members")
          .select("role")
          .eq("workspaceId", project.organizationId)
          .eq("userId", ctx.user.id)
          .single(),
        hasProjectAccess(ctx.supabase, interview.projectId, ctx.user.id),
      ]);
