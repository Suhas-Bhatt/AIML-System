...
import { SessionsTable } from '../../components/session/sessions-table.jsx';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function SessionDetail({
  ...
  const insightsData = summary.data?.insights;
  ...
  const questionEvaluations = Array.isArray(insightsData)
    ? []
    : (insightsData?.questionEvaluations ?? []);
  const researchFindings = Array.isArray(insightsData)
    ? []
  ...
  const sentimentData = summary.data?.sentiment;

  const antiCheatingLog = (summary.data?.antiCheatingLog ?? []);
    ...
    <div className="space-y-6">
      ...
      {summary.isLoading ? (
        ...
        <>
          ...
          >
            {(() => {
              const screenshots = summary.data?.screenshots;
              const avatarUrl =
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                      {whiteboardMsgs.map((msg) => {
                        const label = msg.whiteboardData?.label;
            ...
            {(() => {
              ...
              return (
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    {codeMsgs.map((msg) => {
                      const data = msg.whiteboardData;
                      const label = data?.label ?? "Code Snippet";
