...
import { CodeBlock } from '../../components/code-editor/code-block.jsx';
import {
    CodeEditorCanvas,
} from '../../components/code-editor/code-editor-canvas.jsx';
import { IntervieweeHelpPopover } from '../../components/session/interviewee-help-popover.jsx';
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
import {
    WhiteboardCanvas,
} from '../../components/whiteboard/whiteboard-canvas.jsx';
import { useInterviewRecording } from '../../hooks/use-interview-recording.js';
...
function hasRecentAssistantTranscript(messages, text) {
...
function isExpandedTranscript(previous, next) {
...
function looksLikeInterviewFarewell(text) {
  const normalized = normalizeTranscript(text);
...
function isBriefStandaloneVoiceUtterance(text) {
  const normalized = normalizeTranscript(text);
  if (!normalized) return false;
...
function upsertFinalVoiceTranscript(messages, text, questionId) {
  const nextMessage = {
...
function svgToPngDataUrl(svgDataUrl, maxWidth = 1024) {
...
export function VoiceInterface({
  ...
  const voiceSplitContainerRef = useRef(null);
  const whiteboardRef = useRef(null);
  const codeEditorRef = useRef(null);
  const liveCodeSnapshotRef = useRef(null);
  ...
  const questionContentMapRef = useRef(new Map());
  ...
  const onChatDragStart = useCallback((e) => {
    e.preventDefault();
    const target = e.currentTarget;
    target.setPointerCapture?.(e.pointerId);
  ...
  // -- AI Proctoring Manager (Video Anti-Cheating)
  const AIProctoringManager = dynamic(
    () => import("./ai-proctoring-manager.jsx").then((m) => m.AIProctoringManager),
    { ssr: false }
  );
  ...
  const handleTranscript = useCallback(
    (text, isFinal) => {
      if (isFinal && text.trim()) {
        ...
        setMessages((prev) => upsertFinalVoiceTranscript(prev, text, currentQuestionIdRef.current));
  ...
  const handleAIResponse = useCallback((text) => {
    ...
    setMessages((prev) => {
      ...
      if (last?.role !== "assistant") {
        return [...prev, { id: crypto.randomUUID(), role: "assistant", content: trimmed, source: "voice", questionId: currentQuestionIdRef.current }];
      }
  ...
  const updateCurrentQuestionMutation = trpc.session.updateCurrentQuestion.useMutation();

  const handleQuestionChange = useCallback(async (newIndex) => {
    const qId = interviewContext.questions[newIndex]?.id;
    if (qId) {
      updateCurrentQuestionMutation.mutateAsync({
        sessionId,
        questionId: qId,
      }).catch(err => console.error("Failed to update question progress", err));
    }
  }, [interviewContext.questions, sessionId, updateCurrentQuestionMutation]);
  ...
  const currentQuestionId = interviewContext.questions[voice?.currentQuestionIndex]?.id;
  ...
  }, [voice.isConnected, hasStarted, onStart]);
  ...
  }, [videoMode, voice.isConnected, recording]);
  ...
  }, [videoMode, voice.isListening, recording]);

  // -- Timer (starts when voice connects)
  useEffect(() => {
    if (!voice.isConnected || timerStartedRef.current) return;
    timerStartedRef.current = true;
    setRemainingSeconds(1); // Start from 1
  ...
  const persistDrawing = useCallback(
    async (drawing, snapshotData, imageDataUrl) => {
      try {
  ...
  const renameDrawing = useCallback((drawingId, newLabel) => {
  ...
  const lastCodeAutoSave = useRef(null);
  ...
  const renameCodeSnippet = useCallback((snippetId, newLabel) => {
    const trimmed = newLabel.trim();
    if (!trimmed) return;
  ...
  const [codeSaveStatus, setCodeSaveStatus] = useState("saved");
  ...
  useEffect(() => {
    if (remainingSeconds === null || !durationMinutes || remainingSeconds < durationMinutes * 60 || timerExpiredRef.current) return;
  ...
  const handleSendChat = useCallback((text) => {
    ...
    const msg = { id: crypto.randomUUID(), role: "user", content: trimmed, source: "chat" };
  ...
  const isTimeLow = durationMinutes ? remainingSeconds >= durationMinutes * 60 - 60 : false;
  ...
  const handleSplitDividerDown = useCallback((e) => {
    e.preventDefault();
    const target = e.currentTarget;
    target.setPointerCapture?.(e.pointerId);
  ...
  const handleVoiceTranscriptSplitDown = useCallback((e) => {
    e.preventDefault();
    const target = e.currentTarget;
    target.setPointerCapture?.(e.pointerId);
    splitDragging.current = true;
  ...
  useEffect(() => {
    ...
    if (questionChanged) {
      ...
      if (saved) {
        ...
        const freshDrawings = [{ id: crypto.randomUUID(), label: "Drawing 1", snapshotData: null }];
        const freshSnippets = [{ id: crypto.randomUUID(), label: "Snippet 1", snapshotData: null }];
  ...
  }, [voice.currentQuestionIndex, isCodingQuestion, isWhiteboardQuestion, drawings, activeDrawingIdx, codeSnippets, activeSnippetIdx, currentQVoice]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return shouldShowCompletionScreen ? completionScreen : (
    <div className="relative flex h-screen flex-col overflow-hidden bg-background">
      {/* AI Proctoring (webcam face + phone detection) & Camera PIP */}
      {(videoMode || proctoringEnabled || preview) && (
        <AIProctoringManager 
          sessionId={sessionId}
          stream={videoMode ? recording.cameraStream : undefined}
          enabled={videoMode || proctoringEnabled || preview}
          preview={preview}
        />
      )}
      ...
      <div className="shrink-0 border-b bg-card px-3 py-2 md:px-6 md:py-3">
        ...
        {error && error !== "PERMISSION_DENIED" && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50">
            <Badge variant="destructive" className="px-4 py-2 text-sm font-medium animate-in slide-in-from-top-4">
              {error}
            </Badge>
          </div>
        )}
        ...
        <AlertDialog open={error === "PERMISSION_DENIED"} onOpenChange={(open) => { if (!open) setError(""); }}>
          <AlertDialogContent className="max-w-md text-center">
            <AlertDialogHeader>
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/20 mb-4">
                <MicOff className="h-6 w-6 text-red-600 dark:text-red-500" />
              </div>
              <AlertDialogTitle className="text-xl font-bold">Microphone Access Blocked</AlertDialogTitle>
              <AlertDialogDescription className="text-base mt-2">
                We cannot hear you because microphone access was denied. 
                <br /><br />
                Please click the <strong>🔒 Lock Icon</strong> in your browser's URL bar, toggle Microphone to <strong>Allow</strong>, and then refresh this page to continue your interview.
              </AlertDialogDescription>
            </AlertDialogHeader>
              ...
              {!voice.isConnected && !preview && (
                <Button
                  ...
                  onClick={() => {
                    ...
                    const isVoiceDisabled = process.env.NEXT_PUBLIC_ENABLE_VOICE === "false";
                    ...
                    try {
                      voice.connect().then(() => {
                        ...
                        if (process.env.NEXT_PUBLIC_ENABLE_VOICE !== "false") {
                          voice.greet().catch((err) => console.warn("[voice] greet failed:", err));
                        }
                      }).catch((error) => {
                        console.error(error);
                        clearTimeout(timeout);
                ...
                >
                  ...
                  {isStartingInterview ? "Connecting..." : (process.env.NEXT_PUBLIC_ENABLE_VOICE === "false" ? "Start Text Interview" : "Start Voice Interview")}
                </Button>
              )}
                    ...
                    <ScrollArea className="min-h-0 flex-1">
                      <div className="space-y-3 p-4">
                        {preview ? (
                          ...
                          <>
                            {messages
                              .filter(m => !m.questionId || m.questionId === currentQuestionId || m.role === "assistant")
                              .map((msg) => (
      ...
      {/* -- Camera PIP overlay is now handled globally by AIProctoringManager -- */}
            ...
            <ScrollArea className="min-h-0 flex-1">
              <div className="space-y-3 p-4">
                {!hasStarted ? (
                  ...
                  <>
                    {messages
                      .filter(m => !m.questionId || m.questionId === currentQuestionId)
                      .map((msg) => (
                      <div key={msg.id} className="flex items-start gap-1.5 text-sm">
                        {msg.role === "user" ? (
                          msg.source === "chat"
