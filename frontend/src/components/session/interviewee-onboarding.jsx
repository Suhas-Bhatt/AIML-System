...
import { getMicTestMessage, getSpeechSynthesisLocale } from '../../lib/i18n.js';
import {
    setCameraSkipped,
    setScreenSkipped,
    setStoredScreenStream,
} from '../../lib/media-stream-store.js';
...
import { IntervieweeTourOverlay } from './interviewee-tour-overlay.jsx';
import { IntervieweeTourProvider, useIntervieweeTour } from './interviewee-tour-provider.jsx';
import { VoiceInterface } from './voice-interface.jsx';

const STEPS = [
  { key: "info", label: "Interview Info" },
...
function StepIndicator({ current }) {
  const stepIdxMap = { info: 0, checklist: 1, howItWorks: 2 };
...
function MicCheck({ done, onDone, language, allowSkip = true }) {
  ...
  const relayConnectorRef = useRef(null);
  ...
  const analyzeResponse = useCallback((text) => {
    setPhase("analyzing");
    ...
    const startMicCapture = async () => {
      ...
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          formData.append('audio', audioBlob, 'mic_test' + (mimeType === 'audio/webm' ? '.webm' : '.mp4'));
          
          try {
            const apiBase = process.env.NEXT_PUBLIC_VOICE_API_URL || "";
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          {phase === "idle" && !done && (
            <div className="flex w-full gap-2">
              ...
              {allowSkip && (
                <Button size="sm" variant="outline" onClick={() => setShowSkipDialog(true)}>
                  Skip
                </Button>
              )}
            </div>
  ...
  const header = (
    <header className="sticky top-0 z-50 flex h-14 items-center border-b bg-card px-4 sm:px-6">
      <div className="flex items-center gap-1">
        ...
        <span className="font-heading text-base font-bold tracking-[2px]">AIML SYSTEM</span>
      </div>
    </header>
  );
  ...
  return (
    <div className="flex min-h-screen flex-col bg-muted/30">
      ...
      <div className="mx-auto w-full max-w-2xl flex-1 space-y-4 px-4 pb-8">
        ...
        <div className="flex flex-col items-center justify-center gap-3 pt-4 relative">
          <div className="flex items-center gap-3">
          ...
          <Button 
            variant="ghost" 
            size="sm" 
            className="text-xs text-muted-foreground h-7 hover:bg-muted/50" 
            onClick={handleComplete}
          >
            Skip Checks & Go to Interview (Dev)
          </Button>
        </div>
