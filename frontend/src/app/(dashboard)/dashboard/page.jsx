...
import { StaggerContainer, StaggerItem } from '../../../components/ui/fade-in.jsx';
...
export default function DashboardPage() {
  ...
  const dashboardQuery = trpc.interview.dashboardStats.useQuery(
  ...
  const isPending = orgLoading || projectLoading || (!!projectId && dashboardQuery.isPending);
  ...
  if (dashboardQuery.error || interviewsQuery.error) {
    return (
  ...
  const isEmpty = !projectId || !data || data.totalInterviews === 0;

  if (isEmpty) {
    return (
      <StaggerContainer className="space-y-8" staggerDelay={0.1}>
        {/* Header */}
        <StaggerItem direction="down" className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">Welcome to AIML Interview System</h1>
            <p className="text-muted-foreground">No interview data available yet. Create your first interview to get started.</p>
          </div>
          <Button
          ...
          <Card className="border border-dashed p-8 text-center bg-muted/20">
            <CardContent className="flex flex-col items-center justify-center py-12">
              <p className="text-muted-foreground text-lg font-medium">
                Analytics will be available after candidates complete interviews.
              </p>
            </CardContent>
          </Card>
        </StaggerItem>
  ...
  return (
    <StaggerContainer className="space-y-8" staggerDelay={0.1}>
      ...
      <Card className="transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
        ...
        <CardContent>
          {projectLoading || (!!projectId && interviewsQuery.isLoading) ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <Skeleton key={i} className="h-16 w-full" />
              ))}
            </div>
          ) : interviewsQuery.data?.interviews.length === 0 ? (
            <div className="py-12 text-center">
