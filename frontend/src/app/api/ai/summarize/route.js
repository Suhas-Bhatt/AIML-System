...
export async function POST(req) {
  ...
  try {
    const { data: interviewSession } = await supabaseAdmin
      .from("sessions")
      .select(
        `*, candidate:candidates(*), interview:interviews!inner(title, userId, projectId, objective, language, assessmentCriteria, questions(text, order, type)), messages(contentType, whiteboardData, whiteboardImageUrl, role, content)`,
      )
      .eq("id", sessionId)
