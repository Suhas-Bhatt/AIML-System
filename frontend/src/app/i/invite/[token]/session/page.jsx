...
export default function InviteSessionPage() {
  ...
  useEffect(() => {
    ...
    if (candidate.data) {
      const session = candidate.data.session;
      if (!session) {
        router.replace(`/i/invite/${token}`);
      }
    }
  ...
  const session = candidate.data.session;
  const interview = candidate.data.interview;
  ...
  if (!onboardingDone) {
    return (
      <IntervieweeOnboarding
        ...
        questionTypes={(interview.questions ?? []).map((q) => q.type)}
        onComplete={() => {
          if (interview.antiCheatingEnabled) {
    ...
    return (
      <>
        ...
        <FullscreenEnforcement
          enabled={!!interview.antiCheatingEnabled}
        ...
        <VoiceInterface
          ...
          proctoringEnabled={!!interview.antiCheatingEnabled}
        />
      </>
    );
  }
  ...
  return (
    <>
      ...
      <FullscreenEnforcement
        ...
        onTerminated={() => setFullscreenTerminated(true)}
      />
      {!!interview.antiCheatingEnabled && (
        <AIProctoringManager sessionId={session.id} enabled />
      )}
      <ChatInterface
        sessionId={session.id}
        interview={{
