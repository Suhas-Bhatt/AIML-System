...
class ProctoringService {
  ...
  _startTabSwitchDetection(sessionId) {
    // OPTIMIZATION: Store handlers for cleanup
    this._visHandler = () => {
      if (document.hidden) {
    ...
    // OPTIMIZATION: Store blur handler for cleanup
    this._blurHandler = () => {
      this._post(`/api/sessions/${sessionId}/event`, {
        type:      'TAB_SWITCH',
        timestamp: Date.now() / 1000,
    ...
    // OPTIMIZATION: Store focus handler for cleanup
    this._focusHandler = () => {
      // Optional: Log when user returns to tab
  ...
  _stopTabSwitchDetection() {
    // OPTIMIZATION: Properly remove all event listeners
    if (this._visHandler) {
